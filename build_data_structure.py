...
def generate_trigram_freq_dict(max_size_output, words):
    ...
    if (not os.path.isfile(c.trigrams_path)):
        ...
        for key, value in sorted(output.items(), key=lambda item: item[1], reverse=True):
            if (len(sorted_output) >= max_size_output):
                break
            sorted_output[key] = value

        h.save_obj(sorted_output, "tri_gram")
    else:
        sorted_output = h.load_obj("tri_gram")
...
def generate_word_freq_dict(max_size_output, words):
    if not os.path.isfile(c.word_freq_path):
        ...
        h.save_obj(sorted_output, "word_freq")
    else:
        sorted_output = h.load_obj("word_freq")
...
def get_training_data(number_of_vectors_from_each_source, tri_freq, word_freq):
    ...
    if not os.path.isfile(c.training_data):
        ...
        training_data.extend(
            generate_error_word_features(c.ocr_output_OcropusArgus, c.truthArgus, number_of_vectors_from_each_source, tri_freq, word_freq,
                                         c.error_words_OcropusArgus, 'Argus'))
        print("Added words (5/10)")
        training_data.extend(
            generate_error_word_features(c.ocr_output_OcropusGrepect, c.truthGrepect, number_of_vectors_from_each_source, tri_freq, word_freq,
                                         c.error_words_OcropusGrepect, 'Grepect'))
        print("Added words (6/10)")
        training_data.extend(
            generate_error_word_features(c.ocr_output_TesseractArgus, c.truthArgus, number_of_vectors_from_each_source, tri_freq, word_freq,
                                         c.error_words_TesseractArgus, 'Argus'))
        print("Added words (7/10)")
        training_data.extend(
            generate_error_word_features(c.ocr_output_TesseractGrepect, c.truthGrepect, number_of_vectors_from_each_source, tri_freq, word_freq,
                                         c.error_words_TesseractGrepect, 'Grepect'))
        print("Added words (8/10)")
        training_data.extend(
            generate_error_word_features(c.ocr_output_ABBYYArgus, c.truthArgus, number_of_vectors_from_each_source, tri_freq, word_freq,
                                         c.error_words_ABBYYArgus, 'Argus'))
        print("Added words (9/10)")
        training_data.extend(
            generate_error_word_features(c.ocr_output_ABBYYGrepect, c.truthGrepect, number_of_vectors_from_each_source, tri_freq, word_freq,
                                         c.error_words_ABBYYGrepect, 'Grepect'))
        print("Added words (10/10)")

        h.save_obj(training_data, "training_data")
    else:
        training_data = h.load_obj("training_data")
